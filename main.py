...
def download_model_from_lfs(owner, repo, branch, path, local_path='model.pth'):
    """
    Downloads a file (including LFS) from GitHub using the GitHub API.
    """
...
def predict_urgency(text):
    ...
    with torch.no_grad():
        ...
        class_label = 'urgency keywords present' if output > 0.5 else 'no urgency keywords present'
        return {
            'probability': output,
            'class': class_label
        }
...
if __name__ == "__main__":
    test_text = "This is your last chance to WIN cash. Click this link"
    result = predict_urgency(test_text)
    print(f"Input text: {test_text}")
    print(f"Prediction probability: {result['probability']:.4f}")
    print(f"Class prediction: {result['class']}")
